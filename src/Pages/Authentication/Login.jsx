...
const Login = () => {
  ...
  const handleLogin = async (data) => {
    ...
    // Step 1: Try staff/MySQL login first
    try {
      ...
      if (response.data.success) {
        ...
        return; // Stop here — don't try Firebase
      }
    } catch (err) {
      // If it's a 401 (wrong password for a real staff account), show error and stop
      // Don't fall through to Firebase for staff emails
      if (err.response?.status === 401) {
        // Could be staff with wrong password, OR just not a staff email.
        // We still try Firebase below to cover regular customers.
      } else if (err.response?.status === 500) {
        setErrorMessage("Server error. Please try again.");
        setIsLoading(false);
        return;
      }
      // Network errors or 404 → fall through to Firebase
    }

    // Step 2: Try Firebase login (for regular customers)
    try {
      await loginUser(data.email, data.password);
      navigate("/");
    } catch (firebaseError) {
      // Check if it was a staff email that failed bcrypt (wrong password)
            ...
            <div className="card-body p-6 md:p-8">
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-base-content/60 text-sm mt-2">
                  Customers, staff, and team members
                </p>
              </div>
              ...
              <div className="text-center mt-4 pt-4 border-t border-base-300">
                <p className="text-xs text-base-content/50">
                  Are you an admin?{" "}
                  <Link to="/admin/login" className="link link-secondary font-semibold">
                    Admin Login →
                  </Link>
                </p>
              </div>
